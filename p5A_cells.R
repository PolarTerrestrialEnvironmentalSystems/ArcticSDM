...
sorted_df$Category[grepl("taiga, eurasian", sorted_df$Group)] <- "taiga, eurasian"
sorted_df$Category[grepl("taiga, nearctic", sorted_df$Group)] <- "taiga, nearctic"
...
#### bar plot ####
a <- ggplot(summary_table_long, aes(x = Count_Type, y = Value, fill = Category)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  scale_fill_manual(values = c(taiga_eu, taiga_ne, tundra)) +
  labs(title = "A) timeline constrained",
       x = "",
       y = "colonized cells",
...
#### combined ####
library(cowplot)
combined <- plot_grid(a, bc, nrow = 2)
