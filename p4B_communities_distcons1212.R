...
#### y4 ####
  ##### load new data ####
  membercl <- read.csv(glue::glue("//smb.isipd.dmawi.de/projects/bioing/user/roschw/cellclusters_distcons/membercl_cells_y4s3_distcons.csv"), sep =",")
 ...
 ##### plotting ####
  ...
  y = 3
  ##### load new data ####
  membercl <- read.csv(glue::glue("//smb.isipd.dmawi.de/projects/bioing/user/roschw/cellclusters_distcons/membercl_cells_y3s3_distcons.csv"), sep =",")
  ...
  membercl <- membercl %>%
    mutate(new_group = case_when(
      ...
      group == 1 ~ "taiga, inner", #ne
      group == 6 ~ "tundra'",
      group == 14 ~ "taiga, outer", #eu
      group == 13 ~ "taiga, eurasian, E",
      TRUE ~ as.character(group)  # Keep the original value as a string if none of the above conditions match
    ))
  ...
  y = 2
  ##### load new data ####
  membercl <- read.csv(glue::glue("//smb.isipd.dmawi.de/projects/bioing/user/roschw/cellclusters_distcons/membercl_cells_y2s3_distcons.csv"), sep =",")
  ...
  y = 1
  #### load new data ####
  membercl <- read.csv(glue::glue("//smb.isipd.dmawi.de/projects/bioing/user/roschw/cellclusters_distcons/membercl_cells_y1s3_distcons.csv"), sep =",")
  ...
  membercl <- membercl %>%
    mutate(new_group = case_when(
      group == 0 ~ NA,
      group == 1 ~ "tundra",
      group == 7 ~ "taiga, eurasian",
      group == 11 ~ "taiga, outer",
      group == 18 ~ "taiga, inner",
  ...
  getwd()
  setwd("C:/Users/roschw001/Documents/R/SDM/SDM/ArcticSDMserver/ArcticSDM/cellclustering/cellclusters_distcons")
...
#### legend ####
  ...
  plotlegend <- ggplot(cc) +
    ...
    # Apply custom colors
    theme_minimal() +
    labs(title = glue::glue("y{y}s3 distcons dispersal"), color = "cell communities")+
