...
df2 <- df2 %>%
  mutate(label = ifelse(x == 1| x == 4, node, NA))


df2 <- df2 %>%
  mutate(label = case_when(
    label == "taiga, nearctic''" ~ "boreal forest, nearctic''",
    label == "taiga, nearctic'''" ~ "boreal forest, nearctic'''",
    label == "taiga, eurasian, W" ~ "boreal forest, palearctic, W",
    label == "taiga, eurasian, E'" ~ "boreal forest, palearctic, E'",
    label == "taiga, eurasian" ~ "boreal forest, palearctic",
    label == "taiga, nearctic" ~ "boreal forest, nearctic",
    TRUE ~ as.character(label)
  ))


sankey_res <- ggplot(df2, aes(x = as.factor(year), 
                          next_x = next_x, 
                          node = node, 
                          next_node = next_node,
                          fill = factor(node),
