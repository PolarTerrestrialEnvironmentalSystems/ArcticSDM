...
save(New, file = "C:/Users/roschw001/Documents/R/SDM/SDM/ArcticSDMserver/ArcticSDM/hypotheses_results/plot1 data/New_cells.rda")



newSm <- New %>% filter(cells>0) %>% group_by(scenario, year, disp) %>%

  summarise(median = median(cells, na.rm = T),

            mean = mean(cells, na.rm = T),

            sd = sd(cells, na.rm = T),

            lower_50 = quantile(cells, probs = 0.25, na.rm = T),

            lower_95 = quantile(cells, probs = 0.05, na.rm = T),

            upper_50 = quantile(cells, probs = 0.75, na.rm = T),

            upper_95 = quantile(cells, probs = 0.975, na.rm = T)
...
pl1 <- ggplot(newSm %>% mutate(disp = factor(disp, levels=c("sig5","sig1"))), 
 ...
 # facet_wrap(~disp, labeller = labeller(disp = c("sig5" = "unconstrained","sig1" = "dispersal constrained"))) +
  ...
  labs(x = "", y = "new colonizations") + #\u0394 2010") +
...
median(s1$median/s5$median)
mean(s1$median/s5$median)
median(s1$median)
median(s5$median)
361/2450
