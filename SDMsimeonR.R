...
##### Ensemble model #####
models <- stack(glm_mod, gam_mod, mars_mod, maxent_mod)
...
pred_stars_glm$grd_map[!is.na(pred_stars$grd_map)]  <- predict(glm_mod, newdata = pred_pts)

pred_stars_gam <- grd_out
pred_stars_gam$grd_map[!is.na(pred_stars$grd_map)]  <- predict(glm_mod, newdata = pred_pts)

pred_stars_mars <- grd_out
pred_stars_mars$grd_map[!is.na(pred_stars$grd_map)]  <- predict(glm_mod, newdata = pred_pts)


#plot
ggplot() +
  geom_stars(data = pred_stars_glm) +
