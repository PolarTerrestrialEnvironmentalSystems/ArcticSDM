...
buf <- presbb %>%
  st_buffer(3000) %>% st_union()


  
result <- fut %>% st_as_sf(.) %>% st_crop(buf) %>% st_distance(., presbb) %>%
  apply(., 1, min, na.rm = TRUE)
#get only large numeric


a = as.data.frame(result)
#pr <- st_as_stars(presbb)
#
#futsf <- st_as_sf(fut)
cen <- presbb %>%
  st_buffer(3000) %>% st_centroid() %>% st_union()

dist = st_distance(futcrop, presb) %>%
...
pts <- st_sfc(st_point(c(1,2)), st_point(c(3,4)))
pts_sf <- st_sf(a = 1:2, geometry = pts)

# Convert to stars object 
pts_stars <- st_as_stars(pts_sf)

# Check dimensions
st_dimensions(pts_stars)
