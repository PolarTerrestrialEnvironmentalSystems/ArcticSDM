...
#### y4 ####
  ##### load new data ####
  membercl <- read.csv(glue::glue("//smb.isipd.dmawi.de/projects/bioing/user/roschw/cellclusters_timeline_sigma1/membercl_cells_y4s3_sigma1.csv"), sep =",")
 ...
 ##### plotting ####
  ...
  ##### load new data ####
  membercl <- read.csv(glue::glue("//smb.isipd.dmawi.de/projects/bioing/user/roschw/cellclusters_timeline_sigma1/membercl_cells_y3s3_sigma1.csv"), sep =",")
  unique(membercl$group)
  ...
  ##### load new data ####
  membercl <- read.csv(glue::glue("//smb.isipd.dmawi.de/projects/bioing/user/roschw/cellclusters_timeline_sigma1/membercl_cells_y2s3_sigma1.csv"), sep =",")
  unique(membercl$group)
  ...
  plot1 <- ggplot() +
    ...
    theme_minimal() +
    ggtitle("A) community clusters, constrained", subtitle = "2010")+
    labs(fill = "Cell Communities") +  # Update labels #title = "2010", 
    theme(
  ...
  ggsave(combined_plot_res, file ="4plot.png", dpi = 900)
