...
E <- ggplot(data = dif, aes(x = Category, y = delta * 0.001, fill = Category)) +
  geom_boxplot(color = "black", outlier.size = 1, outlier.shape = 16) +
  labs(x = "",  y = "distance [km]", title = "new cells") +
  theme_minimal() +
  labs(title = expression(paste("E) ", Delta, " 2100 constrained"))) + 
...
print(combined_plot)
P <- add_sub(combined_plot, "communities", hjust = 0.3)
CE <- ggdraw(P)
print(CE)
